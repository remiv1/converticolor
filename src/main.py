...
from color_converter import ColorConverter, ColorHarmony, ContrastChecker
...
class RoundedFrame(tk.Canvas):
    ...
    def __init__(self, parent: tk.Widget, bg_color: str = "#FFFFFF", radius: int = 15, **kwargs: Any) -> None:
    ...
    def _draw_rounded(self, event: Optional[tk.Event] = None) -> None:
        ...
        self.create_arc(0, 0, 2*r, 2*r, start=90, extent=90, fill=self.bg_color, outline=self.bg_color, tags="rounded")  # type: ignore
        self.create_arc(w-2*r, 0, w, 2*r, start=0, extent=90, fill=self.bg_color, outline=self.bg_color, tags="rounded")    # type: ignore
        self.create_arc(0, h-2*r, 2*r, h, start=180, extent=90, fill=self.bg_color, outline=self.bg_color, tags="rounded")  # type: ignore
        self.create_arc(w-2*r, h-2*r, w, h, start=270, extent=90, fill=self.bg_color, outline=self.bg_color, tags="rounded")  # type: ignore

        self.create_rectangle(r, 0, w-r, h, fill=self.bg_color, outline=self.bg_color, tags="rounded")
        self.create_rectangle(0, r, w, h-r, fill=self.bg_color, outline=self.bg_color, tags="rounded")
...
class ConvertiColorApp:
    ...
    def _setup_style(self) -> None:
        ...
        style.configure(self.STYLE_TITLE, font=(self.FONT_MAIN, 16, 'bold'), background=self.BG_COLOR)
        style.configure(self.STYLE_SUBTITLE, font=(self.FONT_MAIN, 10), background=self.BG_COLOR, foreground='#666')
        style.configure(self.STYLE_SECTION, font=(self.FONT_MAIN, 11, 'bold'), background=self.BG_COLOR)
        style.configure(self.STYLE_VALUE, font=(self.FONT_MONO, 11), background=self.BG_COLOR)
    ...
    def _create_widgets(self) -> None:
        ...
        scrollbar = ttk.Scrollbar(container, orient="vertical", command=self.canvas.yview)  # type: ignore
        ...
        subtitle = ttk.Label(main_frame, text="Convertisseur de couleurs multi-formats", style=self.STYLE_SUBTITLE)
        ...
        self.input_entry = ttk.Entry(entry_frame, textvariable=self.input_value, font=('Consolas', 12))
        ...
        # Placeholder
        self.placeholder_label = ttk.Label(input_frame, text="Format: #RRGGBB", style=self.STYLE_SUBTITLE)
        ...
        for fmt, (name, _) in self.FORMATS.items():
            ...
            entry = ttk.Entry(row, textvariable=value_var, font=('Consolas', 11), state='readonly', width=25)
        ...
        self.harmony_canvas = tk.Canvas(harmony_frame, height=60, bg=self.BG_COLOR, highlightthickness=0)
        ...
        # --- Section contraste ---
        contrast_frame = ttk.LabelFrame(main_frame, text=" Vérificateur de contraste WCAG ", padding="10")
        ...
        # Aperçu contraste
        self.contrast_preview = tk.Canvas(contrast_frame, height=50, highlightthickness=1, highlightbackground="#ccc")
        self.contrast_preview.pack(fill=tk.X, pady=5)
        
        # Résultats contraste
        self.contrast_result = ttk.Label(contrast_frame, text="Ratio: -- | WCAG: --", style='Value.TLabel')
